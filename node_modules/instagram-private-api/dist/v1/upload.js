...
class Upload extends Resource {
    static photo(session, streamOrPathOrBuffer, uploadId, name, isSidecar) {
        const data = Buffer.isBuffer(streamOrPathOrBuffer)
            ...
            : helpers_1.Helpers.pathToStream(streamOrPathOrBuffer);
        const compresion = {
            lib_name: 'jt',
            lib_version: '1.3.0',
            quality: '92',
    ...
    static video(session, videoBufferOrPath, photoStreamOrPath, isSidecar, fields) {
        ...
        const request = new request_1.Request(session);
        return helpers_1.Helpers.pathToBuffer(videoBufferOrPath).then(buffer => {
            const duration = _getVideoDurationMs(buffer);
            if (duration > 63000)
                throw new Error(`Video is too long. Maximum: 63. Got: ${duration / 1000}`);
            fields = fields || {};
            ...
            return request
                ...
                return Promise.mapSeries(chunks, (chunk, i) => _sendChunkedRequest(session, uploadData.params.uploadUrl, uploadData.params.uploadJob, sessionId, chunk.data, chunk.range, isSidecar))
                    .then(results => {
                    ...
                    .then(uploadData => Upload.photo(session, photoStreamOrPath, uploadData.uploadId, 'cover_photo_', isSidecar).then(() => uploadData));
            });
        });
    }
    static album(session, medias, caption, disableComments) {
        ...
        medias.forEach(media => {
            ...
            if (media.type === 'photo') {
                uploadPromises.push(Upload.photo(session, media.data, undefined, undefined, true).then(payload => Promise.resolve(Object.assign({}, { uploadId: payload.params.uploadId }, media))));
            }
            if (media.type === 'video') {
                uploadPromises.push(Upload.video(session, media.data, media.thumbnail, true).then(payload => Promise.resolve(Object.assign({}, payload, media))));
            }
        });
